...
const secuirtyMiddleware  = async(req: Request, res: Response, next: NextFunction) =>
{
    ...
    try {
        ...
        switch(role)
        {
            case "admin":
            limit = 20;
            message = "Admit request limit exceeded(20 per minute)";
            break;
            case "teacher":
            case "student":
            limit = 10;
        ...
        {
            return res.status(403).json({error:"Forbidden", message: "Automated requests are forbidden"})
...
export default secuirtyMiddleware;